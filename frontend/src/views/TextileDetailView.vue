...
const checkIfInFavorites = async (textileId) => {
  try {
    // Utilisez la route correcte: /api/favorites
    const response = await api.get('/favorites');
    
    // Analyse de la réponse selon votre structure
    const favorites = response.data?.favorites || response.data || [];
    
    // Vérifiez si le textile est dans les favoris
    isInFavorites.value = favorites.some(fav => 
      fav.textile_id === textileId || fav.id === textileId
    );
  } catch (error) {
    console.error('Erreur lors de la vérification des favoris:', error);
    // Ne pas afficher d'erreur à l'utilisateur pour cette fonctionnalité non critique
  }
};
...
const handleFavorite = async () => {
  ...
  try {
    if (isInFavorites.value) {
      // Utiliser la route correcte pour supprimer: /api/favorites/:textileId
      await api.delete(`/favorites/${textile.value.id}`);
      ...
      // Utiliser la route correcte pour ajouter: /api/favorites/:textileId
      await api.post(`/favorites/${textile.value.id}`); // Notez qu'on n'envoie pas de body ici
...
const checkIfInFavorites = async (textileId) => {
  ...
  try {
    console.log(`Vérification des favoris pour le textile ID: ${textileId}`);
    // Utilisez le bon chemin d'API
    ...
    // Rechercher si ce textile est dans les favoris
    isInFavorites.value = favorites.some(fav => 
      (fav.id === textileId || fav.textile_id === textileId)
    );
    
    console.log(`Textile ${textileId} en favoris: ${isInFavorites.value}`);
  } catch (error) {
    console.error('Erreur lors de la vérification des favoris:', error);
...
const handleFavorite = async () => {
  ...
  try {
    const textileId = textile.value.id;
    console.log(`Action favoris pour textile ID: ${textileId}`);
    ...
    if (isInFavorites.value) {
      ...
      // Ajouter aux favoris
      await api.post(`/users/favorites/${textileId}`);
      isInFavorites.value = true;
      Swal.fire({
        position: 'top-end',
        icon: 'success',
...
const fetchTextileDetails = async () => {
  ...
  try {
    ...
    // Vérifier si le textile est dans les favoris (si l'utilisateur est connecté)
    if (authStore.isUserAuthenticated) {
      await checkIfInFavorites(textileId);
    }
